...
- **DB**: week03에서 docker-compose로 띄운 PostgreSQL(`documents` 테이블). 없으면 아래 3단계에 시드 SQL을 제공한다.
...
week03의 `documents` 테이블이 있으면 건너뛴다. 없으면 시드:
...
**확인**: `psql "$DOCPILOT_DB_DSN" -c "SELECT count(*) FROM documents;"` → `3` 이상.
...
def search_documents_db(keyword: str, limit: int = 5) -> dict:
    """documents 테이블에서 제목/본문에 keyword가 포함된 행을 검색한다."""
    # 입력검증
...
REAL_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "docpilot DB의 documents 테이블을 키워드로 검색한다.",
            "parameters": {
                "type": "object",
                "properties": {
...
curl -s -X POST http://localhost:8000/agent \
  ...
  -d '{"query": "documents 테이블에서 요금제를 찾아 그 금액을 USD로 환산하면 얼마야?"}' | python -m json.tool
```
...
def documents_search(keyword: str, limit: int = 5) -> dict:
    """docpilot DB의 documents 테이블을 키워드로 검색한다."""
...
- documents_search: docpilot DB의 documents 테이블을 키워드로 검색한다.
...
**확인**: Claude Desktop 재시작 후 입력창의 도구(🔨) 아이콘에 `docpilot-tools`가 보이고, "documents에서 환불 정책 찾아줘"라고 하면 `documents_search`가 호출된다.
