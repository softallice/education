...
- **지난주 산출물**: week08 RAG 서비스 (`docpilot`의 `/ask`, 벡터DB 검색 함수). 이번 주는 그 검색 함수를 도구로 감싼다. RAG가 아직 없으면 이 문서의 **fallback 검색 함수**로 대체해 그대로 진행할 수 있다.
...
try:
    # week08에서 만든 검색 함수를 그대로 도구로 승격한다.
    # (프로젝트 구조에 맞게 경로를 조정하세요.)
    from app.rag import search_documents as _rag_search  # type: ignore
except Exception:  # week08이 없거나 import 실패 시 fallback
    _FAKE_DOCS = [
        "docpilot의 부가세율은 10%로 계산합니다.",
        "환불 정책: 구매 후 7일 이내 전액 환불이 가능합니다.",
        "docpilot 프리미엄 요금제는 월 29,000원입니다.",
    ]

    def _rag_search(query: str, top_k: int = 3) -> list[str]:
        """fallback: 아주 단순한 부분 문자열 검색 (week08 벡터검색 대체)."""
        hits = [d for d in _FAKE_DOCS if any(t in d for t in query.split())]
        return (hits or _FAKE_DOCS)[:top_k]
...
### 4부 · LangGraph 버전 (같은 agent를 프레임워크로)
...
| `rag_search`가 fallback만 씀 | week08 `app.rag.search_documents` import 실패 | import 경로를 실제 week08 모듈에 맞게 수정 |
...
- [ ] week08 RAG 검색이 도구로 연결됐다(또는 fallback으로 흐름 확인).
